...
def create_spark_session():
    """
        Initializes the spark session with hadoop config
    """
    spark = SparkSession \
        .builder \
...
def process_song_data(spark, input_data, output_data):
    """
        Given sparkSession object, the input path of songs dataset, and an S3 output path,
        reads the jsons from songs dataset, and transforms into two tables (songs, artists)
        and saves the result in the S3 output path in parquet format.
    """
...
def process_log_data(spark, input_data, output_data):
    """
    Given sparkSession object, the input path of songs dataset, and an S3 output path,
    reads the jsons from log dataset, and transforms into three tables (users, time, songplays)
    and saves the result in the S3 output path in parquet format.
    """
...
def main():
    """
    Calls create_spark_session, process_song_data and process_log_data.
    """
